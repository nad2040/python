...
rotateT = [[cos(theta), -sin(theta), 0], [sin(theta), cos(theta), 0], [0, 0, 1]]
moveT = [[1,0,0],[0,1,0],[deltaX,deltaY,1]]
...
transformT = multMatrix(rotateT, moveT)

def transformXY(pairxy, T) :
    newX = pairxy[0]*T[0][0] + pairxy[1]*T[1][0] + T[2][0]
    newY = pairxy[0]*T[1][0] + pairxy[1]*T[1][1] + T[2][1]
    return (newX, newY)
